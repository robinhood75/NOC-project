...
def get_graph(graph_name, dim, k=None):
    ...
    :return: graph, percolation threshold
    """
    if graph_name == "square_lattice":
        return nx.grid_2d_graph(dim, dim), 0.5
    elif graph_name == "1d_array":
        return nx.grid_graph(dim=dim), 1
    elif graph_name == "cayley_tree":
        assert k is not None
        return nx.full_rary_tree(r=k, n=dim), 1 / (k - 1)
    else:
        raise ValueError(f"Unknown graph name {graph_name}")


def predict_pandemic_threshold(pc, simulation):
    sir = Epidemic.get_sir_values(t_array=np.arange(0, simulation.max_t, simulation.dt),
                                  lambda_=simulation.lambda_,
                                  mu=simulation.mu,
                                  n=simulation.n)
